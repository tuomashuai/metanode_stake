...
contract Pool is IPool {
    ...
    /// @inheritdoc IPool
    uint256 public override feeGrowthGlobal0X128; //记录从创建到现在，每个流动性累计产生的 token0 的手续费
    /// @inheritdoc IPool
    uint256 public override feeGrowthGlobal1X128; //记录从创建到现在，每个流动性累计产生的 token1 的手续费
    ...
    function _modifyPosition(
        ...
        amount0 = SqrtPriceMath.getAmount0Delta(
            sqrtPriceX96, // 当前价格
            TickMath.getSqrtPriceAtTick(tickUpper), // 区间上限价格
            params.liquidityDelta // 流动性变化
        );

        amount1 = SqrtPriceMath.getAmount1Delta(
            TickMath.getSqrtPriceAtTick(tickLower), // 区间下限价格
            sqrtPriceX96, // 当前价格
            params.liquidityDelta // 流动性变化
        );
        ...
        uint128 tokensOwed0 = uint128(
            FullMath.mulDiv(
                feeGrowthGlobal0X128 - position.feeGrowthInside0LastX128, // 手续费增长量
                position.liquidity, // 用户流动性
                FixedPoint128.Q128 // 精度调整 (2^128)
            )
        );
    ...
    function balance0() private view returns (uint256) {
        //使用token0.staticcall进行静态调用，保证只读数据，防止恶意合约回调
        (bool success, bytes memory data) = token0.staticcall(
            abi.encodeWithSelector(IERC20.balanceOf.selector, address(this))
        );
        //data.length >= 32,保证返回数据是uint256类型，防止返回错误数据导致解码失败，利用防御性编程思想
    ...
    function collect(
        ...
        // 获取当前用户的 position   在同一个池子同一个用户，只会有一个position
    ...
    // 交易中需要临时存储的变量
    struct SwapState {
        // 尚未被处理的指定输入/输出数量剩余值 ，这是用户请求交换的原始数量（amountSpecified）的剩余部分。
        int256 amountSpecifiedRemaining;
        // 交易已经实际完成的交换部分所对应的另一侧资产的数量 ，它也是一个有符号整数，其符号与 amountSpecifiedRemaining 相反
        int256 amountCalculated;
        // 当前的现货价格，这是池子当前时刻的平方根价格
    ...
    function swap(
        ...
        bool zeroForOne, //如果从 token0 交换 token1 则为 true，从 token1 交换 token0 则为 false
        int256 amountSpecified, //指定金额
        uint160 sqrtPriceLimitX96, //价格平方根
        bytes calldata data
